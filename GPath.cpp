...
void updateQBounds(const GPoint pts[3], float& minX, float& minY, float& maxX, float& maxY) {
...
void updateCBounds(const GPoint pts[4], float& minX, float& minY, float& maxX, float& maxY) {
   const float epsilon = 1e-6f;
   ...
   while (auto verbOpt = iter.next(pts)) {
       ...
       switch (verb) {
           ...
           case GPathVerb::kQuad:
               updateQBounds(pts, minX, minY, maxX, maxY);
               break;


           case GPathVerb::kCubic:
               updateCBounds(pts, minX, minY, maxX, maxY);
               break;
